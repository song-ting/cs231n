...
from cs231n.layer_utils import affine_relu_dropout_forward, affine_relu_dropout_backward
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params['W2']
        b2 = self.params['b2']
        fc_relu_out, affine_relu_cache = affine_relu_forward(X, W1, b1)  # affine - relu forward
        scores, affine_cache = affine_forward(fc_relu_out, W2, b2)  # output layer: affine forward
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        # dclass_input shape (N, C)
        # loss, dclass_input = softmax_loss(scores, y)
        # loss += 0.5 * self.reg * (np.sum(W2 ** 2) + np.sum(W1 ** 2))
        # grads['W2'] = hid_output.T.dot(dclass_input) + self.reg * W2  # shape (H, C)
        # grads['b2'] = np.sum(dclass_input, axis=0)  # shape (C,)
        #
        # dhid_out = dclass_input.dot(W2.T)  # shape (N, H)
        # dhid_input = relu_backward(dhid_out, hid_input_cache)  # shape (N, H)
        # grads['W1'] = X.T.dot(dhid_input) + self.reg * W1  # shape (D, H)
        # grads['b1'] = np.sum(dhid_input, axis=0)  # shape (H,)

        loss, dscores = softmax_loss(scores, y)  # compute the softmax layer's loss and backforward
        loss += 0.5 * self.reg * (np.sum(W2 ** 2) + np.sum(W1 ** 2))
        dfc_relu_out, grads['W2'], grads['b2'] = affine_backward(dscores, affine_cache)
        grads['W2'] += self.reg * W2  # shape (H, C)

        dX, grads['W1'], grads['b1'] = affine_relu_backward(dfc_relu_out, affine_relu_cache)
        grads['W1'] += self.reg * W1  # shape (H, C)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
